...
namespace NuClear.ValidationRules.Querying.Host.Controllers
{
    ...
    public class ReleaseController : ApiController
    {
        ...
        public ReleaseController(VersioningService versioningService, ValidationResultRepositiory repository, ValidationResultFactory factory)
        {
        ...
        public async Task<IHttpActionResult> Post([FromBody]ApiRequest request, [FromUri]Guid stateToken)
        {
            ...
            var validationResults = _repository.GetResults(CheckMode.Release, versionId, request.OrderIds, request.ProjectId, request.ReleaseDate, request.ReleaseDate.AddMonths(1));
            var result = _factory.GetValidationResult(CheckMode.Release, validationResults);
            return Ok(result);
        }

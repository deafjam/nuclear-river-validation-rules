...
namespace NuClear.ValidationRules.Querying.Host.Controllers
{
    ...
    public class PrereleaseController : ApiController
    {
        ...
        public PrereleaseController(VersioningService versioningService, ValidationResultRepositiory repository, ValidationResultFactory factory)
        {
        ...
        public async Task<IHttpActionResult> Post([FromBody]ApiRequest request, [FromUri]Guid stateToken)
        {
            ...
            var validationResults = _repository.GetResults(CheckMode.Prerelease, versionId, request.OrderIds, request.ProjectId, request.ReleaseDate, request.ReleaseDate.AddMonths(1));
            var result = _factory.GetValidationResult(CheckMode.Prerelease, validationResults);
            return Ok(result);
        }
